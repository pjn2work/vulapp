...
def log_access(response):
    """Write a Werkzeug-style access line for every request into LOG_BUFFER.
    Werkzeug does this automatically when using its dev server, but on PythonAnywhere
    (uWSGI) it never runs, so we generate the line ourselves to keep the buffer complete."""
    timestamp = datetime.now().strftime('%d/%b/%Y %H:%M:%S')
    proto = request.environ.get('SERVER_PROTOCOL', 'HTTP/1.0')
    line = (
        f'{request.remote_addr} - - [{timestamp}] '
        f'"{request.method} {request.path} {proto}" {response.status_code} -'
    )
